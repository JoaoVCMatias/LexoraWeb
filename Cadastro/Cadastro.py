...
def cadastro():
    ...
    .cad-label {{ color: {LABEL_COLOR}; font-size:14px; margin-bottom:6px; font-weight: 600;}}
    ...
    .cad-btn {{ width:100%; height:52px; border-radius:12px; font-size:19px; font-weight:700; margin-bottom:16px;}}
    .cad-btn2 {{ width:100%; height:52px; border-radius:12px; font-size:17px; color:{SUBTLE}; border:1px solid {FIELD_BORDER}; background:#fff;}}
    ...
    def campo_senha(ph, nome_img_eye, nome_img_eyef):
        with ui.element('div').classes('cad-row-box'):
            ui.image('cadeado.png').classes('cad-icon')
            campo = ui.input('', password=True, placeholder=ph).props('autocomplete=off').classes('cad-field')
            eye_state = {'show': False}
            eye_img = ui.image(nome_img_eyef).classes('eye-clicavel')
            def toggle_eye():
                eye_state['show'] = not eye_state['show']
                eye_img.set_source(nome_img_eye if eye_state['show'] else nome_img_eyef)
    ...
    with ui.row().classes('w-full justify-center items-center min-h-screen'):
        with ui.column().classes('items-center').style('max-width:650px; width:100%; margin-top: 30px;'):
            ui.image('logo.png').style('margin-bottom: 8px; max-width: 120px;')
            ui.label('CRIE SUA CONTA').style(f'color: {PRIMARY_BLUE}; font-size: 22px; font-weight: bold; margin: 2px 0 4px 0; text-align:center;')
            ui.label('Leva menos de um minuto para começar gratuitamente!').style(f'color: {SUBTLE}; font-size: 13px; margin-bottom:28px; text-align:center')
            with ui.element('div').classes('cad-card'):
                ...
                with ui.element('div').classes('cad-field-wrap'):
                    with ui.element('div').classes('cad-row-box'):
                        ui.image('user.png').classes('cad-icon')
                ...
                with ui.element('div').classes('cad-field-wrap'):
                    with ui.element('div').classes('cad-row-box'):
                        ui.image('mail.png').classes('cad-icon')
                ...
                ui.button('CRIAR CONTA', on_click=criar_conta, color=PRIMARY_BLUE).classes('cad-btn')
                ui.button('JÁ POSSUO CONTA', color='white').classes('cad-btn2')

    ui.image('ImageM.png').classes('sideimg').style('position:fixed; right:60px; bottom:28px; max-width:350px; max-height:380px; z-index:1;')
